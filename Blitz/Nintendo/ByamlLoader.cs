...
namespace Nintendo
{
    public static class ByamlLoader
    {
        ...
        public static dynamic LoadByamlDynamic(Stream stream)
        {
        ...
        public static dynamic LoadByamlDynamic(byte[] ramByaml)
        {
            using (MemoryStream stream = new MemoryStream(ramByaml))
            {
                return LoadByamlDynamic(stream);
            }
        }

        public static T LoadByamlSerialized<T>(Stream stream)
        {
            // Get the byaml settings
            ByamlSettings settings = PrepareByaml(stream);
        ...
        public static T LoadByamlSerialized<T>(byte[] ramByaml)
        {
            // Load the BYAML from a MemoryStream
            using (MemoryStream stream = new MemoryStream(ramByaml))
            {
                // Read the byaml
                return LoadByamlSerialized<T>(stream);
            }
        }
